        ...
        for path, key, val in self.json_to_path_generator(json_input):
            ...
            if path != prev_path:  # need to construct tha `path` string only of it changed from the last one
                ...
                for i, p in enumerate(path_suffix):
                    ...
                    delim = ':\n' if not is_list else ''
                    str_path_lst.append(
                        '{tabs}**{path_value}**{delim}'.format(tabs=tabs, path_value=path_value, delim=delim))
            ...
            str_lst.append(
                '{path}{tabs}***{key}***: {val}'.format(path=str_path, tabs=full_tabs, key=key,
                                                        val=flattenCell(val, is_pretty)))
    ...
    def get_results_summary(results, errors):
        ...
        for res in results:
            ...
            command = {'command': res.command,
                       'args': res.args}
            comment = res.result.get('Contents', 'No contents found, see entry.') if isinstance(res.result,
                                                                                                dict) else None
        ...
        def _implement_retry(self, retries=0,
            ...
            try:
                method_whitelist = "allowed_methods" if hasattr(Retry.DEFAULT,
                                                                "allowed_methods") else "method_whitelist"  # type: ignore[attr-defined]
                whitelist_kawargs = {
...
def create_indicator_result_with_dbotscore_unknown(indicator, indicator_type, reliability=None,
    ...
    '''
    if not context_prefix and (
            indicator_type is DBotScoreType.CUSTOM or not DBotScoreType.is_valid_type(indicator_type)):
...
def get_fetch_run_time_range(last_run, first_fetch, look_back=0, timezone=0, date_format='%Y-%m-%dT%H:%M:%S'):
    ...
    demisto.debug("lb: fetch start time: {}, fetch end time: {}".format(last_run_time.strftime(date_format),
...
def filter_incidents_by_duplicates_and_limit(incidents_res, last_run, fetch_limit, id_field):
    ...
    demisto.debug('lb: Number of incidents before filtering: {}, their ids: {}'.format(len(incidents_res),
                                                                                       [incident_res[id_field] for
                                                                                        incident_res in incidents_res]))
    ...
    demisto.debug('lb: Number of incidents after filtering: {}, their ids: {}'.format(len(incidents_res),
                                                                                      [incident[id_field] for incident
                                                                                       in incidents]))
...
def remove_old_incidents_ids(found_incidents_ids, current_time, look_back):
    ...
    for inc_id, addition_time in found_incidents_ids.items():
        ...
        if current_time - addition_time <= deletion_threshold_in_seconds:
            ...
            demisto.debug(
                'lb: Adding incident id: {}, its addition time: {}, deletion_threshold_in_seconds: {}'.format(inc_id,
                                                                                                              addition_time,
                                                                                                              deletion_threshold_in_seconds))
        else:
            demisto.debug(
                'lb: Removing incident id: {}, its addition time: {}, deletion_threshold_in_seconds: {}'.format(inc_id,
                                                                                                                addition_time,
                                                                                                                deletion_threshold_in_seconds))
    demisto.debug('lb: Number of new found ids: {}, their ids: {}'.format(len(new_found_incidents_ids),
                                                                          new_found_incidents_ids.keys()))
